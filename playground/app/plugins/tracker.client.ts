...
export default defineNuxtPlugin(() => {
  const requestHeaders: Record<string, string> | undefined = undefined;
  // To attach custom headers to all tracker requests:
  // const requestHeaders = { Authorization: 'Bearer my-token', 'X-App': 'my-app' };
  ...
  function init(mode: 'history' | 'hash') {
    dispose?.();
    const adapter: RuntimeAdapter = createBrowserAdapter({ mode, headers: requestHeaders });
